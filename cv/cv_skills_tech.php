<?php function bandSkillsAndTech() { ?>

  <div class="row">

                <div class="col band-info">    
                  <p class="i2"><b><?php lang("Desarrollo Web Full Stack","As FullStack developer");?></b></p>
                  <p class="i2"><?php lang("API Rest, GIT, Web Frameworks, HTML5, CCS3, Javascript");?></p>
                </div>

                <div class="col band-info">    
                  <p class="i2"><b><?php lang("Como desarrollador Java","As java developer");?></b></p>
                  <p class="i2"><?php lang("Java Desktop, Java 8 Full Stack, Spring Boot, Hibernate, JPA, Maven, JSF");?></p>
                  <p class="i2"></p>
                </div>

                <div class="col band-info">    
                  <p class="i2"><b><?php lang("Como desarrollador PHP","As PHP developer");?></b></p>
                  <p class="i2"><?php lang("Laravel, Symfony Framework");?></p>
                  <p class="i2"></p>
                </div>

                <div class="col band-info">    
                  <p class="i2"><b><?php lang("Como desarrollador C#","As C# developer");?></b></p>
                  <p class="i2"><?php lang("C# .NET, .NET Core, Web Forms, ASPX, Full Stack");?></p>
                  <p class="i2"></p>
                </div>

                <div class="col band-info">    
                  <p class="i2"><b><?php lang("Como desarrollador C++","As C++ developer");?></b></p>
                  <p class="i2"><?php lang("Desarrollo C++, Assembler, Detección de problemas de manejo de memoria","C++ development, assembler, memory management problems detection");?></p>
                  <p class="i2"></p>
                </div>

                <div class="col band-info">    
                  <p class="i2"><b><?php lang("Como desarrollador Microsoft Dynamics","As Microsoft Dynamics developer");?></b></p>
                  <p class="i2"><?php lang("Desarrollo Microsoft Dynamics for Finance and Operations, Análisis de performance","Microsoft Dynamics for Finance and Operations development, performance analysis");?></p>
                  <p class="i2"></p>
                </div>

                <div class="col band-info">    
                  <p class="i2"><b><?php lang("Misceláneas","Other skills and activities");?></b></p>
                  <p class="i2"><?php lang("Métodos ágiles, SCRUM, prototipado rápido, análisis funcional, análisis de performance, optimización SQL, SQL Server, MySQL, PostgreSQL, diseño y análisis de arqutitectura de software, 
                  reingeniería, deploy a producción, detección y remoción de cuellos de botella, mantenimiento de sistemas productivos","Agile methods, SCRUM, rapid prototyping, functional analysis, performance analyisis, SQL optimization, analysis and design of software architecture,  
                  reengineering, deployment to production, analysis and removal of bottlenecks,, maintenance of systems in production");?></p>
                  <p class="i2"></p>
                </div>

              </div>
<?php } ?>